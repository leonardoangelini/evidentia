...
export function buildLlmDocumentBlocks(dataset: DocumentDataset, analysis: Analysis, generatedAt: string): Block[] {
  ...
  blocks.push({ type: 'bullets', items: [
    ...
    `Grandi inserimenti: ${m.insertions.numberOfLargeInsertions} (soglia ${m.insertions.thresholdWords} parole; il più grande ${formatInt(m.insertions.largestInsertionWords)} parole, ${formatInt(m.insertions.totalWordsInLargeInsertions)} parole in totale)`,
